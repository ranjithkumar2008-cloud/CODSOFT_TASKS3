...
# ---------------------------------------------------------------------------
# Data collected from https://books.toscrape.com/ (catalogue pages 1-3, 60 books)
# Title / Price / Availability are real values read from the live site.
# ---------------------------------------------------------------------------
book_list = [
    ...
    ("Foolproof Preserving: A Guide to Small Batch Jams, Jellies, Pickles, Condiments, and More", 30.52, "In stock"),
    ...
    ("The Pioneer Woman Cooks: Dinnertime", 56.41, "In stock"),
    ("The Past Never Ends", 56.50, "In stock"),
    ("The Natural History of Us (The Fine Art of Pretending #2)", 45.22, "In stock"),
]

df = pd.DataFrame(book_list, columns=["Title", "Price", "Availability"])
...
plt.hist(df["Price"], bins=10, color="#4C72B0", edgecolor="white")
plt.title("Book Price Distribution (books.toscrape.com)")
plt.xlabel("Price (£)")
plt.ylabel("Count")
plt.tight_layout()
...
price_bins = pd.cut(df["Price"], bins=[0, 20, 30, 40, 50, 60],
                     labels=["<£20", "£20-30", "£30-40", "£40-50", "£50-60"])
price_bins.value_counts().sort_index().plot(kind="bar", color="#55A868")
plt.title("Book Count by Price Band")
plt.xlabel("Price band")
plt.ylabel("Number of books")
plt.tight_layout()
plt.savefig("price_bands.png", dpi=150)
plt.close()

print("\nFiles saved: books_data.csv, books_data.xlsx, price_distribution.png, price_bands.png")
